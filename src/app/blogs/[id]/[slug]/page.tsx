...
// import { ArrowLeft, Loader2, Share2, Mail, PenLine } from "lucide-react"
...
//       case "copy":
//         await navigator.clipboard.writeText(`${title}\n${description}\n${url}`)
//         toast.success("Blog link with image preview copied!")
//         return
//       default:
//         return
//     }
...
//                       <div className="flex items-center gap-2 text-bold text-purple-700 text-xs sm:text-sm font-medium tracking-wide">
...
//                       <div className="flex items-center gap-2 text-purple-700 text-xs sm:text-sm font-bold tracking-wide">
...
//                       <div className="flex items-center gap-2 gap-2 text-purple-700 text-xs font-bold tracking-wide">
//                         <PenLine className="h-4 w-4 text-black" />
//                         <span>Neelima Kumari</span>
//                       </div>
//                     </div>
...
// app/blogs/[id]/[slug]/page.tsx
// Server component — NO "use client" here.
// Fetches the blog on the server and renders the actual article content
// directly into the HTML. Only the interactive pieces (share buttons,
// animations) live in the client component below.
      ...
      <Script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            datePublished: blog.date,
            dateModified: blog.date,
            author: { "@type": "Person", name: "Neelima Kumari" },
            publisher: {
              "@type": "Organization",
              name: "Aspiration Matters",
              logo: {
            ...
            mainEntityOfPage: {
              "@type": "WebPage",
              "@id": `https://aspirationmatters.com/blogs/${id}/${slug}`,
            },
          }),
        }}
      />

      {/* All the actual article text/markup lives inside this client
          component, but it's called here with `blog` already fetched —
          so it still renders server-side on first load. The "use client"
          directive only makes the HYDRATED interactivity (motion, share
          menu, etc.) client-run; the initial HTML still contains the
          full content because we're passing it in as a prop, not
          fetching it in a useEffect. */}
