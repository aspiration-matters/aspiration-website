...
export const Vortex = ({
    ...
    const particlePropsRef = useRef<Float32Array>(new Float32Array(particleCount * 9))
    const noise3D = useRef(createNoise3D(Math.random))
    ...
    const initParticle = useCallback((i: number) => {
        const randRange = (n: number) => -n + rand(n * 2) // ← move inside
    ...
    const updateParticle = useCallback((i: number, ctx: CanvasRenderingContext2D) => {
        ...
        const [x, y, vx, vy, life, ttl, speed, radius, hue] = props.slice(i, i + 9)

        const angle = noise3D.current(x * xOff, y * yOff, tick.current * zOff) * noiseSteps * TAU
        const nvx = lerp(vx, Math.cos(angle), 0.5)
        const nvy = lerp(vy, Math.sin(angle), 0.5)
        const x2 = x + nvx * speed
        const y2 = y + nvy * speed

        drawParticle(x, y, x2, y2, life, ttl, radius, hue, ctx)

        props.set([x2, y2, nvx, nvy, life + 1, ttl, speed, radius, hue], i)

        if (checkBounds(x2, y2, canvas)) initParticle(i)
    }, [initParticle, TAU])
    ...
    const drawParticles = useCallback((ctx: CanvasRenderingContext2D) => {
        const props = particlePropsRef.current
        for (let i = 0; i < props.length; i += particlePropCount) {
            updateParticle(i, ctx)
        }
    }, [updateParticle])
